<script lang="ts">
    ...
    let inputValueString: string = "" //Inputbox text
    let equationArray: string[] = [] //Array in which the pressed buttons are being pushed into
    let equationHistory = [] //Array which includes Objects saving Formulas and results
    let x: number = 0 //self explanatory
    let y: number = 0 //self explanatory
    let lastFormula //Object which includes the last used Formula. Planning on getting rid of this and finding a better solution
    let openForDelete: boolean = false //Is used to determine if last value of input field is the last result so when a button is pressed afterwards it will be cleared.
    let testOperator: string //testytesting
    
    function calculate(operator: string){ //does the calculationpart
    ...
    function getXAndY(operator: string) {  //does the getting X and Y part
    ...
    const equationCapture = (operandOrNumber: string) => { //Does the Collecting data part and works weird :(
